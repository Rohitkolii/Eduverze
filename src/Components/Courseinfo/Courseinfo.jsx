...
const Courseinfo = () => {
  ...
  const CurrentCourseData = useSelector(
    (store) => store.coursedetailreducer.courseDetail.data
  );
  ...
  const PurchaseCourse = async (id) => {
    ...
    const OrderDetail = {
      payment: {
        method: "UPI",
        status: "success",
        transactionId: "APLPHABETA",
      },
    };
    try {
      const response = await fetch(
        `http://localhost:3001/api/orders/addneworder/${id}`,
        {
          method: "POST",
          headers: {
      ...
      <div className={Styles.Courseinfoinner}>
        <div className={Styles.Courseinfocol1}>
          ...
          <img
            ...
            src={
              `http://localhost:3001/${CurrentCourseData?.course_thumbnail}` ||
              ""
            }
            alt=""
          />
        ...
        <div className={Styles.couseinfoCard}>
          <div>
            ...
            {CurrentCourseData?.course_points ? (
              <ul>
                {CurrentCourseData?.course_points?.map((point, i) => (
                  <li key={i}>{point}</li>
                ))}
              </ul>
            ) : (
            ...
            <p className={Styles.price}>
              ₹{CurrentCourseData?.course_price || ""} <del> ₹79.99</del>
            </p>
            <button onClick={() => PurchaseCourse(CurrentCourseData._id)}>
              Enroll Now
            </button>
            {/* <button onClick={()=> navigate(`/courses/checkout/${CurrentCourseData._id}`)}>Enroll Now</button> */}
          </div>
        </div>
      </div>
    </section>
